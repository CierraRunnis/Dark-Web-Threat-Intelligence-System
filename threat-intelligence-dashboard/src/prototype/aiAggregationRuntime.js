...
const runStatus = (run) => String(run?.analysis_status ?? run?.status ?? '').toLowerCase()
...
export async function hydrateAiAggregationScreen({ root, signal: parentSignal, route } = {}) {
  ...
  const managePoll = () => { const next = state.runs.some((run) => ['queued', 'running'].includes(runStatus(run))) ? ACTIVE_POLL_MS : IDLE_POLL_MS; if (state.timer && state.pollMs === next) return; if (state.timer) window.clearInterval(state.timer); state.pollMs = next; state.timer = window.setInterval(poll, next) }
  ...
  const loadRuns = async () => { state.runs = listFrom(await api.listRuns(), ['runs', 'items', 'results']); if (state.currentRun) state.currentRun = { ...state.currentRun, ...(state.runs.find((run) => runId(run) === runId(state.currentRun)) || {}) }; else state.currentRun = state.runs[0] || null; renderRuns() }
  const openRun = async (id, scroll = true) => { try { const detail = await api.getRun(id); const index = state.runs.findIndex((run) => runId(run) === id); if (index >= 0) state.runs[index] = { ...state.runs[index], ...detail }; else state.runs.unshift(detail); state.currentRun = state.runs.find((run) => runId(run) === id) || detail; renderRuns(); if (scroll) $('[data-ai-report-section]')?.scrollIntoView({ behavior: 'smooth', block: 'start' }) } catch (reason) { showError(reason) } }
  async function poll() { if (state.polling || state.disposed) return; state.polling = true; try { const [health] = await Promise.allSettled([api.health(), loadRuns()]); if (health.status === 'fulfilled') { state.health = health.value; updateHealth() }; if (['queued', 'running'].includes(runStatus(state.currentRun))) await openRun(runId(state.currentRun), false) } finally { state.polling = false } }
