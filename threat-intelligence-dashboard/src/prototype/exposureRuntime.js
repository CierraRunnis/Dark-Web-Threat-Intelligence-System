...
import { openPlatformRemoteLogin } from '@/prototype/exposureDataRuntime'
const exposureTimeouts = new Set()
...
function trackTimeout(callback, delay) {
  const handle = window.setTimeout(() => {
    exposureTimeouts.delete(handle)
    callback()
  }, delay)
  exposureTimeouts.add(handle)
  return handle
}

function clearExposureTimers() {
  exposureTimeouts.forEach((handle) => window.clearTimeout(handle))
  exposureTimeouts.clear()
}


function showToast(message) {
  let toast = document.querySelector('[data-exposure-toast]')
  if (!toast) {
    toast = document.createElement('div')
    toast.className = 'toast'
    toast.dataset.exposureToast = ''
  ...
  showToast.timer = trackTimeout(() => toast.classList.remove('show'), 2400)
}
...
export function initializeExposurePrototype(root) {
  ...
  function setupTabs() {
    ...
    const settingsModule = ['netdisk', 'library', 'code'].includes(requestedSettingsModule) ? requestedSettingsModule : ''
    if (settingsModule) {
  ...
  function setupCollectorControl() {
    ...
    const syncMutationButton = (button) => {
      const collectorAction = button.dataset.collectorAction || ''
      const codeAction = button.dataset.codeAction || ''
      const adminOnly = ['bot-save', 'bot-test'].includes(collectorAction)
        || ['github-save', 'github-delete'].includes(codeAction)
      if (adminOnly && !isCurrentUserAdmin()) {
        button.dataset.adminGated = '1'
        button.disabled = true
        return
      }
      if (button.dataset.adminGated === '1') delete button.dataset.adminGated
    ...
    function renderGithubApp(payload = {}) {
      ...
      ;['#github-app-id', '#github-installation-id', '#github-private-key'].forEach((selector) => {
        const control = $(selector, root)
        if (control) control.readOnly = !canManage
      })
      ;['github-save', 'github-delete'].forEach((action) => {
        const button = $('[data-code-action="' + action + '"]', root)
        if (button) {
          button.disabled = !canManage
          button.title = canManage ? '' : '仅管理员可以修改 GitHub App 配置'
        }
      })
    }
    ...
    async function downloadCodeTermTemplate() {
    ...
    function setTorRouteState(nextState) {
      ...
      trackTimeout(() => route.classList.remove('is-state-change'), 560)
    }
    ...
    function scheduleTorPoll(payload = {}) {
      ...
      torPollTimer = trackTimeout(async () => {
        torPollTimer = 0
    ...
    function renderBot(payload = {}) {
      ...
      const canManage = isCurrentUserAdmin()
      ;['#bot-id', '#bot-secret'].forEach((selector) => {
        const control = $(selector, root)
        if (control) control.readOnly = !canManage
      })
    }
    ...
    root.addEventListener('click', (event) => {
      ...
      if (codeAction === 'term-template') {
        void downloadCodeTermTemplate()
          .then(() => showToast('代码检索词模板已生成'))
          .catch((error) => showToast(error.message || '模板生成失败'))
        return
      }
  ...
  return () => {
    clearExposureTimers()
    document.querySelector('[data-exposure-toast]')?.remove()
  }
}
