...
const activeRemoteLoginDisposers = new Set()
const exposureDataTimers = new Map()

function trackDataTimeout(callback, delay) {
  const handle = window.setTimeout(() => {
    exposureDataTimers.delete(handle)
    callback()
  }, delay)
  exposureDataTimers.set(handle, { kind: 'timeout', cancel: null })
  return handle
}

function trackDataInterval(callback, delay) {
  const handle = window.setInterval(callback, delay)
  exposureDataTimers.set(handle, { kind: 'interval', cancel: null })
  return handle
}

function dataDelay(delay) {
  return new Promise((resolve) => {
    const handle = window.setTimeout(() => {
      exposureDataTimers.delete(handle)
      resolve()
    }, delay)
    exposureDataTimers.set(handle, { kind: 'timeout', cancel: resolve })
  })
}

function clearDataTimer(handle) {
  if (!handle) return
  const timer = exposureDataTimers.get(handle)
  if (timer?.kind === 'interval') window.clearInterval(handle)
  else window.clearTimeout(handle)
  exposureDataTimers.delete(handle)
}

function clearExposureDataTimers() {
  for (const [handle, timer] of exposureDataTimers) {
    if (timer.kind === 'interval') window.clearInterval(handle)
    else window.clearTimeout(handle)
    timer.cancel?.()
  }
  exposureDataTimers.clear()
}
...
function showToast(message) {
  ...
  clearDataTimer(showToast.timer)
  showToast.timer = trackDataTimeout(() => toast.classList.remove('show'), 2600)
}
...
function openCollectorRemoteLogin(site, options = {}) {
  ...
  const closeDialog = async (closeSession = true) => {
    if (closed) return
    closed = true
    activeRemoteLoginDisposers.delete(closeDialog)
    stopPolling()
    browserStream?.close()
    browserStream = null
    streamPath = ''
  ...
  activeRemoteLoginDisposers.add(closeDialog)
  ...
  query(overlay, '[data-browser-save]').addEventListener('click', async () => {
    ...
    try {
      ...
      trackDataTimeout(() => window.location.reload(), 900)
    } catch (error) {
  ...
  ;(async () => {
    try {
      if (options.initialState?.session_id) {
        ...
        refreshTimer = trackDataInterval(refreshState, 3000)
        return
      }
      if (platform === 'changan') {
...
export function disposeExposureDataRuntime() {
  for (const closeDialog of [...activeRemoteLoginDisposers]) {
    void closeDialog()
  }
  activeRemoteLoginDisposers.clear()
  clearExposureDataTimers()
}


export async function hydrateExposurePrototypeScreen({ root, route, file }) {
  if (!root) return
  hydrateAccount(root)
