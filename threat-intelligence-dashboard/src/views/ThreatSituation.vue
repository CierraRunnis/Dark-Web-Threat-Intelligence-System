...
import { useThreatSituationData } from '@/composables/useThreatSituationData'

const DEMO_MODE = import.meta.env.VITE_DEMO_MODE === '1'
const router = useRouter()
const { data } = useThreatSituationData()
...
const industryDistribution = computed(() => {
  const aggregateRows = resolveArraySection('threatExecutiveIndustryDistribution')
  if (aggregateRows.length) {
    return aggregateRows.map((item, index) => ({
      name: normalizeIndustryLabel(item.name),
...
const activeActors = computed(() => {
  const aggregateRows = resolveArraySection('threatExecutiveActiveActors')
  if (aggregateRows.length) {
    return aggregateRows.map((item) => ({
  ...
  return [...grouped.values()]
    ...
    .sort((left, right) => right.value - left.value || right.averageRiskScore - left.averageRiskScore)
    .slice(0, 10)
})
...
function viewEventDetail(row) {
  ...
  router.push({ name: 'EventDetail', params: { eventId: row.id }, query: { module: 'threat_situation' } })
}
