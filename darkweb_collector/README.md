...
首次启动且未设置 `DARKWEB_AUTH_PASSWORD` 时，服务会生成随机管理员密码并保存到
本机私有密码文件，日志只输出文件路径，不输出密码。也可以在启动前显式配置：

```bash
export DARKWEB_AUTH_USERNAME="admin"
export DARKWEB_AUTH_PASSWORD="replace-with-a-strong-password"
```

不要在仓库、启动脚本或截图中保存真实密码。
## 兼容脚本
