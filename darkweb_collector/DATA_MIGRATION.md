...
5. 将镜像目录写入同一个 ZIP64 容器，并逐条核对数据库路径引用确实存在于包内。WSL 中仅大小写不同的代码/文库镜像会追加基于原相对路径的稳定哈希后缀，数据库引用同步更新，避免 Windows 解包时覆盖；没有显式路径字段的暗网核心镜像发生同类冲突时会拒绝打包。
...
2026-08-19 使用 WSL ext4 中两个仅大小写不同且内容不同的 `code_monitoring` HTML 镜像完成回归：迁移包为两个文件生成不同的稳定归档名，数据库路径同步更新；在 Windows 释放并导入 PostgreSQL 后两份内容均保留、路径均位于新镜像根目录，摘要复核通过。对没有显式路径引用的 `dragonforce` 大小写冲突样本按安全边界拒绝打包。
