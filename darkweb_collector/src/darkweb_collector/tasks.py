...
def _release_thread_browser_client() -> None:
    """Best-effort release of this thread's Playwright client.

    Used at the end of each celery task so a thread-pool worker doesn't
    inherit a Playwright instance bound to a different thread on the next
    task — which would trigger
    ...
    try:
        ...
        print(f"[tasks] close_browser_client failed (ignored): {exc}")
...
def crawl_seed(self, site_name: str, force: bool = False) -> dict[str, object]:
    ...
    try:
        ...
        _release_thread_browser_client()
