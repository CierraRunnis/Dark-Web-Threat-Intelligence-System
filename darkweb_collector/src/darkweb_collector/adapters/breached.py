...
from darkweb_collector.sites.breached import parse_breached_detail, parse_breached_list
...
def _section_name(url: str) -> str:
    """`/forums/databases.14/?...` -> `databases`. Falls back to a slug of the path."""
    path = urlparse(url).path.strip("/")
    if path.startswith("forums/"):
        last = path.split("/")[1]
        return last.split(".")[0].replace("-", "_").lower() or "section"
...
class BreachedAdapter(SiteAdapter):
    site_name = "breached"
    ...
    def _is_valid_detail_html(html: str) -> bool:
        ...
        if nul_ratio > 0.01:
            return False
        # XenForo thread pages always carry these markers — used to detect cloudflare/login walls.
        required_markers = (
            'class="message message--post',
            'class="bbWrapper',
            "data-content=\"post-",
            "p-body-pageContent",
        )
    ...
    def collect_seed(self, config: SiteConfig, run_ctx: RunContext) -> SeedResult:
        ...
        for url in config.seed_urls:
            ...
            parsed = parse_breached_list(url, html, max_topics=config.max_topics_per_run)
    ...
    def plan_details(self, seed_result: SeedResult, config: SiteConfig) -> list[DetailTask]:
        ...
        # Round-robin across sections so a single noisy section can't starve others.
    ...
    def collect_detail(
        ...
        for attempt in range(3):
            ...
            if self._is_valid_detail_html(html):
                detail = parse_breached_detail(detail_task.target_url, html)
            ...
            print(
                f"[breached] invalid detail html on attempt {attempt + 1} for "
                f"{detail_task.target_url}; retrying"
            )
            close_browser_client()
        print(f"[breached] skipping invalid detail persist for {detail_task.target_url}")
        return None
