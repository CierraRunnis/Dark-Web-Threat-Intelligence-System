...
def vulnerability_cte(connection=None) -> str:
    if is_postgresql(connection):
        return _VULNERABILITY_CTE.replace("MIN(100,", "LEAST(100,")
    return _VULNERABILITY_CTE
...
def _normalized_where(
    ...
    *,
    ...
    if normalized_days:
        clauses.append(
            f"datetime(COALESCE(NULLIF({prefix}disclosure_time, ''), {prefix}updated_at)) >= datetime(?)"
        )
        boundary = datetime.now(timezone.utc) - timedelta(days=normalized_days)
        parameters.append(boundary.strftime("%Y-%m-%d %H:%M:%S"))
    if str(industry or "").strip():
        clauses.append(sql_case_insensitive_like(connection, f"{prefix}industry"))
        parameters.append(_like(str(industry)))
    if str(region or "").strip():
        clauses.append(
            "(" + sql_case_insensitive_like(connection, f"{prefix}region") + " OR "
            + sql_case_insensitive_like(
                connection, f"json_extract({prefix}event_metadata_json, '$.macro_region')"
            ) + ")"
        )
    ...
    if str(country or "").strip():
        clauses.append(
            "(" + sql_case_insensitive_like(
                connection, f"json_extract({prefix}event_metadata_json, '$.country')"
            ) + " OR " + sql_case_insensitive_like(
                connection, f"json_extract({prefix}event_metadata_json, '$.country_code')"
            ) + ")"
        )
    ...
    if str(attacker or "").strip():
        clauses.append(sql_case_insensitive_like(connection, f"{prefix}attacker"))
    ...
    if str(keyword or "").strip():
        ...
        clauses.append(
            "(" + " OR ".join(
                sql_case_insensitive_like(connection, f"{prefix}{column}")
                for column in (
...
def _query_normalized_rows(
    ...
    **filters: Any,
    ...
    where_clause, parameters = _normalized_where(connection, event_types=event_types, **filters)
...
def _vulnerability_where(
    ...
    *,
    ...
    normalized_days = _positive_int(days)
    if normalized_days:
        clauses.append(f"datetime({prefix}disclosure_time) >= datetime(?)")
        boundary = datetime.now(timezone.utc) - timedelta(days=normalized_days)
        parameters.append(boundary.strftime("%Y-%m-%d %H:%M:%S"))
    if str(severity or "").strip():
    ...
    if str(attacker or vendor or "").strip():
        clauses.append(sql_case_insensitive_like(connection, f"{prefix}vendor"))
        parameters.append(_like(str(attacker or vendor)))
    if str(product or "").strip():
        clauses.append(sql_case_insensitive_like(connection, f"{prefix}product"))
...
def _query_vulnerability_rows(
    ...
    **filters: Any,
    ...
    where_clause, parameters = _vulnerability_where(connection, **filters)
    rows = connection.execute(
        f"""
        {vulnerability_cte(connection)}
...
def _count_vulnerabilities(connection, **filters: Any) -> int:
    ...
    return int(
        connection.execute(
            f"{vulnerability_cte(connection)} SELECT COUNT(*) FROM deduped_vulnerabilities v WHERE {where_clause}",
            parameters,
        ).fetchone()[0]
    )
    ...
    *,
    ...
    with get_readonly_db_connection() as connection:
        ...
        try:
            ...
            where_clause, parameters = _normalized_where(connection, event_types=["ransomware"], **filters)
            actor_ranking = _counter_rows(
                connection,
                f"SELECT attacker, COUNT(*) FROM normalized_intelligence_events n WHERE {where_clause} "
                f"AND TRIM(attacker) <> '' GROUP BY attacker ORDER BY COUNT(*) DESC, {sql_case_insensitive_order(connection, 'attacker')} LIMIT ?",
                parameters,
                6,
            )
            industry_ranking = _counter_rows(
                ...
                f"AND TRIM(industry) <> '' GROUP BY industry ORDER BY COUNT(*) DESC, {sql_case_insensitive_order(connection, 'industry')} LIMIT ?",
                parameters,
                6,
            )
            timeline_rows = connection.execute(
                f"""
                SELECT date(datetime(COALESCE(NULLIF(disclosure_time, ''), updated_at))) AS bucket, COUNT(*)
                FROM normalized_intelligence_events n
                WHERE {where_clause}
                GROUP BY bucket
        ...
        try:
            ...
            categories = [
                ...
                for row in connection.execute(
                    f"""
                    SELECT category FROM normalized_intelligence_events
            ...
            attackers = [
                str(row[0]) for row in connection.execute(
                    f"""
                    SELECT attacker FROM normalized_intelligence_events
                    WHERE event_type = 'data_leak' AND TRIM(attacker) <> ''
                    GROUP BY attacker
                    ORDER BY {sql_case_insensitive_order(connection, 'attacker')}
                    """
                ).fetchall()
            ]
            industries = [
                str(row[0]) for row in connection.execute(
                    f"""
                    SELECT industry FROM normalized_intelligence_events
                    WHERE event_type = 'data_leak' AND TRIM(industry) <> ''
                    GROUP BY industry
                    ORDER BY {sql_case_insensitive_order(connection, 'industry')}
                    """
                ).fetchall()
            ]
    ...
    with get_readonly_db_connection() as connection:
        ...
        try:
            ...
            severity_rows = connection.execute(
                f"{vulnerability_cte(connection)} SELECT effective_severity, COUNT(*) FROM deduped_vulnerabilities v "
                f"WHERE {where_clause} GROUP BY effective_severity",
                parameters,
            ).fetchall()
            totals = connection.execute(
                f"{vulnerability_cte(connection)} SELECT COUNT(*), SUM(is_exploited), SUM(patch_available), "
            ...
            vendor_ranking = _counter_rows(
                connection,
                f"{vulnerability_cte(connection)} SELECT vendor, COUNT(*) FROM deduped_vulnerabilities v "
            ...
            product_ranking = _counter_rows(
                connection,
                f"{vulnerability_cte(connection)} SELECT product, COUNT(*) FROM deduped_vulnerabilities v "
                f"WHERE {where_clause} AND TRIM(product) <> '' GROUP BY product ORDER BY COUNT(*) DESC, product LIMIT ?",
                parameters,
                6,
            )
            timeline_rows = connection.execute(
                f"{vulnerability_cte(connection)} SELECT date(datetime(disclosure_time)), COUNT(*) FROM deduped_vulnerabilities v "
                f"WHERE {where_clause} GROUP BY date(datetime(disclosure_time)) ORDER BY date(datetime(disclosure_time)) DESC LIMIT 30",
                parameters,
            ).fetchall()
        finally:
            connection.rollback()
...
def _fetch_vulnerabilities_by_ids(connection, cve_ids: list[str]) -> dict[str, dict[str, Any]]:
    ...
    rows = connection.execute(
        f"{vulnerability_cte(connection)} SELECT {_VULNERABILITY_COLUMNS} FROM deduped_vulnerabilities "
    ...
    with get_readonly_db_connection() as connection:
        ...
        try:
            ...
            normalized_where, normalized_parameters = _normalized_where(
                connection, event_types=normalized_types, **normalized_filters
            )
            vulnerability_where, vulnerability_parameters = _vulnerability_where(connection, **vulnerability_filters)
            normalized_branch = f"""
