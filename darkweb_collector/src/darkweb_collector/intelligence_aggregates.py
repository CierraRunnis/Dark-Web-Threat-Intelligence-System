...
def aggregate_revision(connection) -> str:
    ...
    value = {
        # This is a public content revision, so it must be identical for an
        # equivalent SQLite snapshot and PostgreSQL release. The physical
        # database identity belongs only in the in-process cache key below.
...
def _revision_cache_identity(connection, revision: str) -> str:
    """Scope an equivalent public revision to its physical connection."""

    return f"{database_identity(connection)}:{revision}"
...
def _normalized_events(connection, start: str, types: list[str], severity: str, keyword: str) -> list[dict[str, Any]]:
    ...
    if keyword:
        ...
        clauses.append("(" + " OR ".join(sql_case_insensitive_like(connection, field) for field in fields) + ")")
...
def _vulnerability_events(connection, start: str, severity: str, keyword: str) -> list[dict[str, Any]]:
    ...
    if keyword:
        ...
        parameters.extend([pattern] * len(fields))
    result = connection.execute(
        f"""{vulnerability_cte(connection)} SELECT * FROM deduped_vulnerabilities
            WHERE {' AND '.join(clauses)}
...
def _document_events(connection, start: str, severity: str, keyword: str) -> list[dict[str, Any]]:
    ...
    if keyword:
        pattern = f"%{_escape_like(keyword)}%"
        fields = ("h.title", "w.name", "w.organization_name", "h.raw_json")
        clauses.append(
            "(" + " OR ".join(sql_case_insensitive_like(connection, field) for field in fields) + ")"
        )
...
def _severity_counts(connection, start: str) -> Counter[str]:
    ...
    queries = (
        ...
        (f"""{vulnerability_cte(connection)} SELECT effective_severity,COUNT(*) FROM deduped_vulnerabilities
...
def build_dashboard_overview(*, days: int = 7, event_type: str = "all", severity: str = "", keyword: str = "") -> dict[str, Any]:
    ...
    with get_readonly_db_connection() as connection:
        ...
        try:
            ...
            key = sha1(json.dumps(
                {
                    "revision": _revision_cache_identity(connection, revision),
                    "days": days,
                    "type": event_type,
                    "severity": severity,
                    "keyword": keyword,
            ...
            vulnerability_count = int(connection.execute(
                f"""{vulnerability_cte(connection)} SELECT COUNT(*) FROM deduped_vulnerabilities
                    WHERE datetime(COALESCE(NULLIF(disclosure_time,''),last_seen_at)) >= datetime(?)""",
                (start,),
            ).fetchone()[0])
            ...
            for row in connection.execute(
                f"""{vulnerability_cte(connection)}
            ...
            vendors = _rows(connection,
                f"""{vulnerability_cte(connection)} SELECT vendor AS name,COUNT(*) AS value FROM deduped_vulnerabilities
                    WHERE datetime(COALESCE(NULLIF(disclosure_time,''),last_seen_at)) >= datetime(?)
...
def build_threat_situation(*, days: int = 30) -> dict[str, Any]:
    ...
    with get_readonly_db_connection() as connection:
        ...
        try:
            ...
            key = sha1(
                f"{_revision_cache_identity(connection, revision)}:{monitoring_revision}:{days}".encode()
            ).hexdigest()
            ...
            vulnerability_industries = int(connection.execute(
                f"{vulnerability_cte(connection)} SELECT {sql_scalar_min(connection, '500', 'COUNT(*)')} FROM deduped_vulnerabilities"
            ).fetchone()[0])
