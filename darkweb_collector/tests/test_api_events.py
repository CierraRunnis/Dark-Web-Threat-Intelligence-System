...
from darkweb_collector.normalizer_service import refresh_pending_normalization
...
class ApiEventsTests(unittest.TestCase):
                    ...
                    "sites": [
                        {
                            ...
                            "enabled": True,
                            "seed_urls": ["https://darkforums.ru/Forum-Databases"],
    ...
    def test_overview_uses_full_data_leak_total_when_event_list_is_limited(self) -> None:
        data_leak_events = [{"category": "databases", "region": "亚洲"}]

        dashboard_summary, data_leak_summary, _ = api_data._build_summary_cards(
            data_leak_events,
            [],
            [],
            [],
            data_leak_total=3001,
        )
        preview_cards = api_data._build_preview_cards(
            data_leak_events,
            [],
            [],
            data_leak_total=3001,
        )
        data_leak_preview = next(item for item in preview_cards if item["route"] == "/data-leak")

        self.assertEqual("3001", dashboard_summary[0]["value"])
        self.assertEqual("3001", data_leak_summary[0]["value"])
        self.assertEqual("3001", data_leak_preview["stats"][0]["value"])
            ...
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        ...
                        url="https://darkforums.ru/thread-acme",
                        author="poster",
                        replies="1",
                        views="10",
                    ...
                    upsert_forum_detail(
                        ...
                        site_name="darkforums",
                        section="databases",
                        topic_url="https://darkforums.ru/thread-acme",
            ...
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        ...
                        section="other_leaks",
                        title="Breach Record",
                        url="https://darkforums.ru/thread-breach",
                        author="poster",
                        replies="1",
                        views="20",
                        published_at="2026-03-17",
                    ...
                    upsert_forum_detail(
                        ...
                        site_name="darkforums",
                        section="other_leaks",
                        topic_url="https://darkforums.ru/thread-breach",
            ...
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        ...
                        url="https://darkforums.ru/Thread-XIAOMI-China-Database-Leaked-Download",
                        author="Knox",
                        replies="1",
                        views="20",
                        published_at="",
                    ...
                    upsert_forum_detail(
                        ...
                        site_name="darkforums",
                        section="databases",
                        topic_url="https://darkforums.ru/Thread-XIAOMI-China-Database-Leaked-Download",
                        content="full detail content",
                        authors="Knox",
    ...
    def test_build_event_detail_hydrates_late_forum_artifacts_without_recollecting(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            topic_url = "https://darkforums.ru/thread-late-artifact"
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        connection,
    ...
    def test_build_event_detail_refreshes_when_first_forum_detail_arrives_after_cache_exists(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            topic_url = "https://darkforums.ru/thread-first-detail"
            event_id = "forum:darkforums:databases:" + api_data._event_hash(topic_url)
            artifact_stem = api_data._event_hash(topic_url)[:10]
    ...
    def test_build_event_detail_prefers_raw_forum_labels_when_normalized_cache_is_stale(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            topic_url = "https://darkforums.ru/thread-victim-refresh"
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        connection,
            ...
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        ...
                        url="https://darkforums.ru/thread-direct",
                        author="poster",
                        replies="1",
                        views="20",
                        published_at="2026-03-17",
                    ...
                    upsert_forum_detail(
                        ...
                        site_name="darkforums",
                        section="databases",
                        topic_url="https://darkforums.ru/thread-direct",
                ...
                event_id = "forum:darkforums:databases:" + api_data._event_hash("https://darkforums.ru/thread-direct")
            ...
            with patch.dict(os.environ, self._env(db_path, config_path), clear=False):
                with get_db_connection() as connection:
                    upsert_forum_topic(
                        ...
                        url="https://darkforums.ru/thread-incomplete",
                        author="poster",
                        replies="1",
                        views="20",
                        published_at="2026-03-17",
                    ...
                    connection.execute(
                        ...
                        (
                            "darkforums",
                            "other_leaks",
                            "https://darkforums.ru/thread-incomplete",
                            "short content",
                            "poster",
                            "",
                            "",
                ...
                event = next(item for item in build_event_records() if item["id"].endswith(api_data._event_hash("https://darkforums.ru/thread-incomplete")))
