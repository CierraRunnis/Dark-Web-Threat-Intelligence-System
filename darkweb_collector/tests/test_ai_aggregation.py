...
def test_settings_can_read_local_flocks_api_token(tmp_path: Path) -> None:
    secret_path = tmp_path / ".secret.json"
    secret_path.write_text(
        json.dumps({"server_api_token": "local-token-value"}),
        encoding="utf-8",
    )
    env = {
        "DARKWEB_COLLECTOR_DB_PATH": str(tmp_path / "collector.db"),
        "DARKWEB_COLLECTOR_OUTPUT_ROOT": str(tmp_path / "output"),
        "FLOCKS_API_KEY": "",
        "FLOCKS_SECRET_FILE": str(secret_path),
    }
    with patch.dict(os.environ, env, clear=False):
        settings = Settings.from_env()
    assert settings.flocks_api_key == "local-token-value"


def test_explicit_flocks_api_key_overrides_secret_file(tmp_path: Path) -> None:
    env = {
        "DARKWEB_COLLECTOR_DB_PATH": str(tmp_path / "collector.db"),
        "DARKWEB_COLLECTOR_OUTPUT_ROOT": str(tmp_path / "output"),
        "FLOCKS_API_KEY": "explicit-token",
        "FLOCKS_SECRET_FILE": str(tmp_path / "missing-secret.json"),
    }
    with patch.dict(os.environ, env, clear=False):
        settings = Settings.from_env()
    assert settings.flocks_api_key == "explicit-token"
