...
class AuthAccountsTests(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        with api_app._auth_lock:
            api_app._auth_sessions.clear()
    def test_missing_password_generates_private_random_password_file(self) -> None:
        password_path = Path(self.tmp_dir.name) / "initial-admin-password.txt"
        with patch.dict(
            os.environ,
            {
                "DARKWEB_AUTH_PASSWORD": "",
                "DARKWEB_AUTH_PASSWORD_FILE": str(password_path),
            },
            clear=False,
        ):
            password = api_app._auth_password()
        self.assertTrue(password_path.is_file())
        self.assertEqual(password, password_path.read_text(encoding="utf-8").strip())
        self.assertGreaterEqual(len(password), 24)
        self.assertNotEqual("123456", password)
        if os.name != "nt":
            self.assertEqual(0, password_path.stat().st_mode & 0o077)
